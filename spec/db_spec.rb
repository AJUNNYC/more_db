describe 'database' do
  ...
  def run_script(commands)
    raw_output = nil
    IO.popen("./db4 test.db", "r+") do |pipe|
  ...
  it 'prints error message when key is not found' do 
    result = run_script ([
      "insert 1 user1 person1@example.com",
  ...
  it 'deletes a row and node is not underfilled' do 
    result = run_script ([
      "insert 1 user1 person1@example.com",
      "insert 2 user2 person2@example.com",
      "insert 3 user3 person3@example.com",
    ...
    expect(result[9...(result.length)]).to match_array([
      "db > Tree:",
      "- leaf (size 7)",
      "  - 1",
      "  - 3",
  ...
  it 'merges 2 leaf nodes into root after deleting' do 
    result = run_script ([
      "insert 1 user1 person1@example.com",
      "insert 2 user2 person2@example.com",
      "insert 3 user3 person3@example.com",
  ...
  it 'merges 2 leaf nodes in an internal node after deleting' do 
    result = run_script ([
      "insert 1 user1 person1@example.com",
      "insert 2 user2 person2@example.com",
      "insert 3 user3 person3@example.com",
    ...
    expect(result[29...(result.length)]).to match_array([
      "db > Tree:",
      "- internal (size 2)",
      "  - leaf (size 13)",
      "    - 1",
  ...
  it 'deletes a leaf node from an internal node' do 
    ...
    script << ".exit"
    result = run_script(script)
    expect(result[36...(result.length)]).to match_array([
      "db > Tree:",
      "- internal (size 1)",
        "  - internal (size 1)",
        "    - leaf (size 13)",
  ...
  it 'deletes an internal node from an internal node' do 
    script = (1..49).map do |i|
      "insert #{i} user#{i} person#{i}@example.com"
    end
    script << "delete 28"
    script << ".btree"
    script << ".exit"
    result = run_script(script)
    expect(result[50...(result.length)]).to match_array([
      "db > Tree:",
      "- internal (size 2)",
      "  - internal (size 1)",
      "    - leaf (size 7)",
  ...
  it 'deletes all rows' do 
    ...
    script << ".exit"
    result = run_script(script)
    expect(result[200...(result.length)]).to match_array([
      "db > Tree:",
      "- leaf (size 0)",
      "db > ",
    ])
